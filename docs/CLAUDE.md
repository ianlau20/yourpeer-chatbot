...
- **Test suite**: 19 pytest files (719 tests) covering all services, routes, edge cases, geolocation, rate limiting, security, privacy, family composition, multi-service extraction, split classifier, taxonomy enrichment, and DB schema/query integration

## Known Gaps / In Progress

- **Multi-intent requests** — extraction and split-classifier routing are complete. Multiple service types are extracted and the first is searched with tone-aware framing. Remaining: service queue (PR 3, offering the next service after results) and LLM extractor multi-service schema (PR 4). See `MULTI_INTENT_PLAN.md`
