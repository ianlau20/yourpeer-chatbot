...
- **Narrative extraction (20+ words)** — long messages are automatically detected and routed through a specialized Sonnet prompt with explicit urgency hierarchy (shelter > medical > food > employment). For narratives, the LLM is fully authoritative — regex does not override service_type, preventing "hospital" from overriding "housing fell through." When LLM is unavailable, a regex fallback re-ranks all detected services by urgency and infers urgency from context clues ("evicted", "just released", "ran away")
...
- **Privacy question handling** — questions about privacy, ICE, police, benefits impact, anonymity, recording, and data deletion are classified as bot questions and answered with specific reassurances. Powered by the `bot_knowledge.py` self-knowledge module with 15 topics organized by priority. Includes 13 privacy/information-handling phrases for the "what happens to my information" question type
- **Bot self-knowledge module** — `bot_knowledge.py` is a single source of truth for both the LLM prompt and static handler. Sources capability data from actual code at runtime: service categories from `slot_extractor.SERVICE_KEYWORDS`, PII types from `pii_redactor._PLACEHOLDERS`, location count from `slot_extractor._KNOWN_LOCATIONS`. 15 topics with priority ordering ensure multi-topic collisions resolve correctly (privacy > location > services). `build_capability_context()` generates the LLM "Facts about yourself" section from live data; `answer_question()` provides keyword-matched static answers
...
- **Implicit service change detection** — during a pending confirmation, any message with a DIFFERENT `service_type` is treated as an implicit correction — no denial keyword needed. Follows the industry standard (Rasa, Microsoft CLU, Dialogflow) of slot-level conflict detection. Handles negation-blind regex: "Not food, I need shelter" extracts both services; the system swaps to the one that differs from pending. Additive intent ("also", "too", "as well") is distinguished from change intent — additive queues the new service rather than replacing the current one
- **LLM "other" service interception** — when the LLM extractor returns `service_type="other"` without a `service_detail` AND the regex extractor also returned None, the system treats it as unrecognized rather than confirming a search for "other services." Distinguishes genuinely unrecognizable requests ("helicopter ride") from legitimate "other" category items ("SNAP benefits")
- **Emotional handling (AVR pattern + Option 3)** — follows the Acknowledge-Validate-Redirect pattern from clinical chatbot research (Woebot, Wysa, DAPHNE). Emotion detection runs before intent classification. Six emotion-specific static responses (scared, sad, rough_day, shame, grief, isolation) validate the particular feeling expressed. **Static scaffold + LLM enhancement (Option 3):** the static response is ALWAYS the base — it's verified to pass all eval criteria. When the LLM is available, it optionally adds ONE personalized sentence (max 15 words) that references the user's specific situation. The enhancement is validated against a 56-item blocklist of service words, soft-push phrases, and steering questions. If invalid, the static response stands alone. This whitelist approach was chosen over validate-and-reject (Option 2) because it eliminates the "soft pushing" gap where the LLM avoids explicit service words but still steers toward services. See [CHATBOT_BEHAVIOR.md — Emotional Handling Design](CHATBOT_BEHAVIOR.md#emotional-handling-design) for the research basis and implementation details
- **Crisis step-down** — when crisis fires on a non-acute category (safety_concern, domestic_violence) and the user has explicit service intent, crisis resources are shown alongside an offer to search for the mentioned service. Service slots are preserved in session so "yes" executes the search without re-entering intake. Acute crisis categories (suicide, medical, trafficking, violence) always show crisis resources only
...
- **Contraction normalization** — `_normalize_contractions()` expands 37 common contractions before phrase matching, so phrase lists only need the "not" form to match all variants ("isn't helpful", "wasnt helpful", "aren't helpful" all match "not helpful")
- **Intensifier stripping** — `_strip_intensifiers()` removes 20 common adverbs (really, very, so, super, extremely, etc.) before phrase matching, so "I'm really scared" matches "i'm scared" without needing explicit "really scared" entries. Combined with contraction normalization, this eliminates the combinatorial explosion of intensifier×contraction×emotion phrase variants
...
- **Multi-service extraction and queue** — the slot extractor detects ALL service types in a message (e.g., "I need food and shelter in Brooklyn" extracts both). The primary service is searched first; additional services are queued and offered sequentially after results ("You also mentioned shelter — would you like me to search for that too?"). Users can accept, decline, change location, or start over at each queue step. Session slots (location, age, family_status) carry over between queued searches
...
- Shame tone has normalizing responses but not a fully distinct handler — shame expressions route to the emotional handler with a shame-specific static response and tone prefix, not a dedicated shame handler with specialized follow-up
