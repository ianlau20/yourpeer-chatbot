...
- **Multi-service extraction** — the slot extractor detects ALL service types in a message (e.g., "I need food and shelter in Brooklyn" extracts both). The primary service is used for the current search; additional services are stored as `additional_services` for future queue-based handling (routing pending PRs 2–4)
